      ...
      {activePost ? (
        <article className="max-w-3xl mx-auto space-y-10">
          ...
          <div className="space-y-4 border-b border-neutral-200/40 dark:border-white/5 pb-8">
            <span className="px-3.5 py-1 text-[10px] font-mono font-bold uppercase tracking-wider rounded-sm bg-[#D12B2B]/10 text-[#D12B2B] border border-[#D12B2B]/20">
              {activePost.category}
            </span>
            <h1 className="text-fluid-h1 font-light font-serif italic text-neutral-900 dark:text-white leading-[1.1]">
              {activePost.title}
            </h1>
          ...
          <div className="prose prose-neutral dark:prose-invert max-w-none text-neutral-700 dark:text-neutral-300 font-sans text-base leading-relaxed space-y-6">
            ...
            {activePost.content.trim().split('\n\n').map((block, index) => {
              ...
              if (isHeader3) {
                return (
                  <h3 key={index} className="text-xl md:text-2xl font-bold font-serif italic text-neutral-900 dark:text-white tracking-tight pt-4">
                    {block.replace('###', '').trim()}
                  </h3>
                );
              }
              ...
              if (isBulletList) {
                return (
                  <ul key={index} className="list-disc list-inside space-y-2 pl-4 text-neutral-600 dark:text-neutral-400 font-sans font-light">
                    {block.split('\n').map((li, liIndex) => (
          ...
          <div className="pt-12 border-t border-neutral-200/40 dark:border-white/5 text-center">
            <button
              ...
              onMouseEnter={() => setCursorMode('hover')}
              onMouseLeave={() => setCursorMode('default')}
              className="px-8 py-3.5 rounded-sm border border-neutral-300 dark:border-neutral-700 hover:border-[#D12B2B] text-xs font-mono tracking-widest uppercase text-neutral-800 dark:text-neutral-200 dark:hover:border-[#D12B2B] transition-colors"
            >
              Back to list index
            </button>
          </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6">
            {JOURNAL_POSTS.map((post) => (
              <div
                ...
                onMouseEnter={() => setCursorMode('hover')}
                onMouseLeave={() => setCursorMode('default')}
                className="group p-8 rounded-sm bg-white/5 dark:bg-[#0A0A0A]/40 border border-neutral-200/40 dark:border-white/5 backdrop-blur-md cursor-pointer hover:border-[#D12B2B]/40 transition-all flex flex-col justify-between gap-6 shadow-md"
              >
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg font-bold font-serif italic text-neutral-900 dark:text-[#F5F5F4] tracking-tight group-hover:text-[#D12B2B] transition-colors duration-300">
                      {post.title}
                    </h3>
                    <p className="text-xs text-neutral-500 dark:text-zinc-400 font-sans font-light leading-relaxed line-clamp-3">
                      {post.excerpt}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-neutral-900 dark:text-white group-hover:text-[#D12B2B] transition-colors">
